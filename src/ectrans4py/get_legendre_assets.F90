...
REAL(KIND=8), DIMENSION(KSLOEN), INTENT(OUT) :: PMU
...
! Local buffers passed to TRANS_INQ must match its precision (JPRB for the
! Gaussian weights / Legendre polynomials, JPRD for the sines of the latitudes);
! the double-precision output arguments are then filled from them.
REAL(KIND=JPRB), DIMENSION(KSLOEN) :: ZGW
REAL(KIND=JPRD), DIMENSION(KSLOEN) :: ZMU
REAL(KIND=JPRB), DIMENSION(KSLOEN/2,KSPOLEGL) :: ZRPNM
...
IF (.NOT. LLSTOP) THEN
  CALL TRANS_INQ(KRESOL=IIDENTRESOL, KNMENG=INMENG, PGW=ZGW, PMU=ZMU, PRPNM=ZRPNM)
  KNMENG=INMENG
  PGW=ZGW
  PMU=ZMU
  PRPNM=ZRPNM
