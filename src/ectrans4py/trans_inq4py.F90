...
REAL(KIND=REAL64), DIMENSION(KSIZEJ), INTENT(OUT) :: PMU, PGW
!
...
IF (.NOT. LLSTOP) THEN
  CALL TRANS_INQ(KRESOL=IRESOL, KGPTOT=IGPTOT, KSPEC=ISPEC, KSPEC2=ISPEC2, &
   & KGPTOTG=IGPTOTG, KSPEC2G=ISPEC2G, KSMAX=ISMAX, KNMENG=INMENG, PMU=ZMU, PGW=ZGW)
  KGPTOT  = IGPTOT
  KSPEC   = ISPEC
  KSPEC2  = ISPEC2
  KGPTOTG = IGPTOTG
  ...
  KNMENG(:) = INMENG(:)
  PMU(:) = ZMU(:)
  PGW(:) = REAL(ZGW(:), KIND=REAL64)
ENDIF
END SUBROUTINE TRANS_INQ4PY


SUBROUTINE TRANS_INQ_VSET4PY(KRETURNCODE, KRESOL, KPRTRW, KMYSETW, KMYSETV)
! Inquire the spectral (wave x V-set) processor-grid position for the 2-D spectral
! decomposition (nprtrv>1): KPRTRW wave-sets, this task's wave-set KMYSETW and V-set
! KMYSETV (both 1-based). nprtrv = NPROC/KPRTRW. Used to build the KVSETSC/KVSETUV
! V-set field map for the V-set-aware transforms/scatter.
USE ISO_FORTRAN_ENV, ONLY: INT64
USE PARKIND1, ONLY: JPIM
IMPLICIT NONE
INTEGER(KIND=INT64), INTENT(OUT) :: KRETURNCODE
INTEGER(KIND=INT64), INTENT(IN)  :: KRESOL
INTEGER(KIND=INT64), INTENT(OUT) :: KPRTRW, KMYSETW, KMYSETV
INTEGER(KIND=JPIM) :: IPRTRW, IMYSETW, IMYSETV
#include "trans_inq.h"
KRETURNCODE = 0
IPRTRW = 0
IMYSETW = 0
IMYSETV = 0
CALL TRANS_INQ(KRESOL=INT(KRESOL,JPIM), KPRTRW=IPRTRW, KMYSETW=IMYSETW, KMYSETV=IMYSETV)
KPRTRW  = IPRTRW
KMYSETW = IMYSETW
KMYSETV = IMYSETV
END SUBROUTINE TRANS_INQ_VSET4PY
