...
CALL GSTATS(1640, 0)
!$OMP PARALLEL DO COLLAPSE(2) SCHEDULE(DYNAMIC) PRIVATE(JGL,JK,ZGTF)
DO JGL = 1, D%NDGL_FS
  DO JK = 1, KF_FS
    ! Fourier transform
    CALL FTDIR(PIN, ZGTF, JGL, JK)

    ! Save Fourier data in FOUBUF_IN
    CALL FOURIER_OUT(ZGTF, KF_FS, JGL, JK)
  ENDDO
ENDDO
!$OMP END PARALLEL DO
CALL GSTATS(1640, 1)
