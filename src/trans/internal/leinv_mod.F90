...
!     Modifications.
!     --------------
!        J.Hague : Oct 2012 DR_HOOK round calls to GEMM:
...
REAL(KIND=JPRB)    :: ZBA(KIFC,(R%NSMAX-KM+2)/2), ZBS(KIFC,(R%NSMAX-KM+3)/2), ZC(KIFC,KDGLU)
CHARACTER(LEN=1) :: CLX
...
IF( KDGLU > 0 ) THEN
  ...
  IF(ILA <= ITHRESHOLD .OR. .NOT.S%LUSEFLT) THEN
    ...
    CALL GEMM('N','T',KIFC,KDGLU,ILA,1.0_JPRB,ZBA,KIFC,S%FA(KMLOC)%RPNMA,KDGLU,0._JPRB,ZC,KIFC)
  ...
  IF(ILS <= ITHRESHOLD .OR. .NOT.S%LUSEFLT ) THEN
    ...
    CALL GEMM('N','T',KIFC,KDGLU,ILS,1.0_JPRB,ZBS,KIFC,S%FA(KMLOC)%RPNMS,KDGLU,0._JPRB,ZC,KIFC)
