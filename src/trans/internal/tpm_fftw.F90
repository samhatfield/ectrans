...
interface EXEC_EFFTW
   module procedure EXEC_EFFTW_DIR_NOBATCH
end interface
...
SUBROUTINE EXEC_EFFTW_DIR_NOBATCH(KRLEN,KCLEN,KOFF,PIN,PREEL,JF)
...
IF (LHOOK) CALL DR_HOOK('EXEC_EFFTW_DIR',0,ZHOOK_HANDLE)

CALL CREATE_PLAN_FFTW(IPLAN_C2R1,-1,KRLEN,1)
ZFFT1P=FFTWF_ALLOC_COMPLEX(INT(KCLEN/2,C_SIZE_T))
CALL C_F_POINTER(ZFFT1P,ZFFT1,[KCLEN])
DO JJ=1,KRLEN
...
IF (LHOOK) CALL DR_HOOK('FFTW_EXECUTE_DFT_R2C',1,ZHOOK_HANDLE2)
DO JJ=1,KCLEN
  PREEL(KOFF+JJ-1)=ZFFT1(JJ)/REAL(KRLEN,JPRB)
ENDDO
CALL FFTWF_FREE(ZFFT1P)

IF (LHOOK) CALL DR_HOOK('EXEC_EFFTW_DIR',1,ZHOOK_HANDLE)
END SUBROUTINE EXEC_EFFTW_DIR_NOBATCH
