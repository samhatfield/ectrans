...
REAL(KIND=JPRB),TARGET  :: PIN(D%NLENGTF,KF_FS)
...
IF (KF_UV > 0 .OR. KF_SCDERS > 0 .OR.  (LATLON.AND.S%LDLL) ) THEN
  ...
  IF (LDIVGP) THEN
    IST = IST+KF_UV
  ENDIF
  IF (KF_UV>0) ZUV => PIN(:,IST:IST+2*KF_UV-1)
  IST = IST+2*KF_UV
  IF (KF_SCALARS>0) ZSCALAR => PIN(:,IST:IST+KF_SCALARS-1)
  IST = IST+KF_SCALARS
  IF (KF_SCDERS>0) ZNSDERS => PIN(:,IST:IST+KF_SCDERS-1)
  IST = IST+KF_SCDERS
  IF (LUVDER) THEN
    ZUVDERS => PIN(:,IST:IST+2*KF_UV-1)
    IST = IST+2*KF_UV
  ENDIF
  IF (KF_SCDERS > 0) THEN
    ZEWDERS => PIN(:,IST:IST+KF_SCDERS-1)
  ENDIF
ENDIF
...
DO JGL = 1, D%NDGL_FS
  IGL = JGL
  CALL FOURIER_IN(PIN,KF_OUT_LT,IGL)
...
!    3.  Fourier transform
  ...
  IF (KF_FS > 0) THEN
    CALL FTINV(PIN,ZGTF,KF_FS,IGL) ! Watch out failures here (Cray CCE 8.6.2 ? Intel 18.0.1 ?)
  ENDIF
ENDDO
!$OMP END PARALLEL DO
CALL GSTATS(1639,1)
