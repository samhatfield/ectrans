...
CONTAINS
  SUBROUTINE LEINV_STRIDES(KF_LEG,IOUT_STRIDES0,IOUT_SIZE,IIN_STRIDES0,IIN_SIZE,&
    ...
    IF (PRESENT(IIN_SIZE)) &
      IIN_SIZE = IIN_STRIDES0*D%OFFSETS_GEMM2(D%NUMP+1)
    IF (PRESENT(IOUT0_STRIDES0)) &
      IOUT_STRIDES0 = ALIGN(2*KF_LEG,A)
    IF (PRESENT(IOUT_SIZE)) &
      IOUT_SIZE = IOUT_STRIDES0*D%OFFSETS_GEMM1(D%NUMP+1)
...
#define HIP_GEMM HIP_DGEMM_GROUPED
...
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(ALLOC:ZINP,ZOUTS,ZOUTA,ZINP0,ZOUTS0,ZOUTA0,PIA) MAP(TO:R,FG,D)
#endif
#ifdef ACCGPU
    !$ACC DATA PRESENT(D,D%MYMS,D%NUMP) &
    !$ACC&     PRESENT(ZINP,ZOUTS,ZOUTA,ZINP0,ZOUTS0,ZOUTA0) &
    !$ACC&     PRESENT(ZAA,ZAS,PIA) &
    !$ACC&     PRESENT(R,R%NSMAX,D%OFFSETS_GEMM2)
#endif
      ...
      DO JK=1,2*KF_LEG
        ...
        IF(KM /= 0)THEN
          ...
          DO J=1,(R%NSMAX-KM+2)/2
            ZINP(JK+(J-1)*IIN_STRIDES0+D%OFFSETS_GEMM2(KMLOC)*IIN_STRIDES0)=PIA(JK,IA+1+(J-1)*2,KMLOC)
          ENDDO
          ...
          DO J=(R%NSMAX-KM+2)/2+1,ALIGN((R%NSMAX-KM+2)/2,A)
            ZINP(JK+(J-1)*IIN_STRIDES0+D%OFFSETS_GEMM2(KMLOC)*IIN_STRIDES0)=0
          ENDDO
#endif
   ...
   ENDIF
    ...
    DO KMLOC=1,D%NUMP
      ...
      NS(KMLOC) = G%NDGLU(KM)
      AOFFSETS(KMLOC) = IIN_STRIDES0*D%OFFSETS_GEMM2(KMLOC)
      BOFFSETS(KMLOC) = D%OFFSETS_GEMM_MATRIX(KMLOC)
      COFFSETS(KMLOC) = IOUT_STRIDES0*D%OFFSETS_GEMM1(KMLOC)
    ENDDO
    IF(IMLOC0(1) > 0) THEN
      NS(IMLOC0(1)) = 0
      KS(IMLOC0(1)) = 0
      ...
      DO JK=1,2*KF_LEG
        ...
        IF(KM /= 0) THEN
          ...
          DO J=1,(R%NSMAX-KM+3)/2
            ZINP(JK+(J-1)*IIN_STRIDES0+D%OFFSETS_GEMM2(KMLOC)*IIN_STRIDES0)=PIA(JK,IS+1+(J-1)*2,KMLOC)
          ENDDO
#if defined(USE_CUTLASS) && defined(USE_CUTLASS_3XTF32)
          ! those are only needed with tensor cores (zinp might contain NaNs!)
          !$ACC LOOP SEQ
