...
CONTAINS
  SUBROUTINE LEDIR_STRIDES(KF_FS,IOUT_STRIDES0,IOUT_SIZE,IIN_STRIDES0,IIN_SIZE,&
    ...
    IF (PRESENT(IOUT_SIZE)) &
        IOUT_SIZE = IOUT_STRIDES0*D%OFFSETS_GEMM2(D%NUMP+1)
    IF (PRESENT(IIN_STRIDES0)) &
        IIN_STRIDES0 = ALIGN(2*KF_FS,A)
    IF (PRESENT(IIN_SIZE)) &
        IIN_SIZE = IIN_STRIDES0*D%OFFSETS_GEMM1(D%NUMP+1)
...
#define HIP_GEMM HIP_DGEMM_GROUPED
    ...
    INTEGER(KIND=JPIM) :: IA, IS, ISL, J, IMLOC0(1)
    ...
    ASSOCIATE(ZAA=>FG%ZAA, ZAS=>FG%ZAS, ZAA0=>FG%ZAA0, ZAS0=>FG%ZAS0)
...
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(ALLOC:ZINPS,ZINPA,ZOUT,ZINPS0,ZINPA0,ZOUT0,POA1) MAP(TO:R,FG,D)
#endif
#ifdef ACCGPU
    !$ACC DATA &
    !$ACC& PRESENT(ZINPS,ZINPA,ZOUT,ZINPS0,ZINPA0,ZOUT0) &
    !$ACC& PRESENT(D,D%MYMS,D%NUMP,R,R%NTMAX,R%NSMAX) &
    !$ACC& PRESENT(ZAA,ZAS,POA1,D%OFFSETS_GEMM1,D%OFFSETS_GEMM2)
#endif
    ...
    DO KMLOC=1,D%NUMP
      ...
      KS(KMLOC) = G%NDGLU(KM)
      AOFFSETS(KMLOC) = IIN_STRIDES0*D%OFFSETS_GEMM1(KMLOC)
      BOFFSETS(KMLOC) = D%OFFSETS_GEMM_MATRIX(KMLOC)
      COFFSETS(KMLOC) = IOUT_STRIDES0*D%OFFSETS_GEMM2(KMLOC)
    ENDDO
    IF(IMLOC0(1) > 0) THEN
      NS(IMLOC0(1)) = 0
      KS(IMLOC0(1)) = 0
      ...
      DO JF=1,2*KF_FS
        ...
        IF (KM /= 0) THEN
          ...
          DO J=1,(R%NSMAX-KM+2)/2
            POA1(JF,IA+1+(J-1)*2,KMLOC) = ZOUT(JF+(J-1)*IOUT_STRIDES0+D%OFFSETS_GEMM2(KMLOC)*IOUT_STRIDES0)
          ENDDO
      ...
      DO JF=1,2*KF_FS
        ...
        IF (KM /= 0) THEN
          ...
          DO J=1,(R%NSMAX-KM+3)/2
            POA1(JF,IS+1+(J-1)*2,KMLOC) = ZOUT(JF+(J-1)*IOUT_STRIDES0+D%OFFSETS_GEMM2(KMLOC)*IOUT_STRIDES0)
          ENDDO
        ELSEIF (MOD(JF-1,2) == 0) THEN
#ifdef ACCGPU
          !$ACC LOOP SEQ
