...
USE CUDA_GEMM_BATCHED_MOD, ONLY : CUDA_SGEMM_BATCHED, CUDA_DGEMM_BATCHED
...
!$OMP& MAP(ALLOC:PAIA) &
!$OMP& MAP(ALLOC:ZAA,ZAS,DZBST,DZCST,DZCAT) &
!$OMP& MAP(ALLOC:POA1,dzbst0,dzcat0,dzbst0,dzcst0)
#endif
...
#ifdef OMPGPU
!$OMP TARGET PARALLEL DO COLLAPSE(3) PRIVATE(KM,KDGLU,ISL,ISKIP) DEFAULT(NONE) &
...
#ifdef OMPGPU
!$OMP TARGET PARALLEL DO COLLAPSE(3) PRIVATE(KM,ISKIP,ILA,IA,ILS) DEFAULT(NONE) &
...
#ifdef OMPGPU
  !$OMP TARGET PARALLEL DO COLLAPSE(2) PRIVATE(KDGLU,ISL) DEFAULT(NONE) &
...
#ifdef OMPGPU
   !$OMP TARGET PARALLEL DO COLLAPSE(2) PRIVATE(ILA,IA,ILS) DEFAULT(NONE) &
...
#ifdef OMPGPU
!$OMP TARGET PARALLEL DO COLLAPSE(3) PRIVATE(KM,ISKIP,ILA,IS,ILS) DEFAULT(NONE) &
...
#ifdef OMPGPU
   !$OMP TARGET PARALLEL DO COLLAPSE(2) PRIVATE(ILA,ILS,IS) DEFAULT(NONE) &
