...
CONTAINS
  SUBROUTINE DIR_TRANS_CTL(KF_UV_G,KF_SCALARS_G,KF_GP,KF_FS,KF_UV,KF_SCALARS,&
    ...
    USE TPM_TRANS,              ONLY: GROWING_ALLOCATION, GROWING_GAM_ALLOCATION
    ...
    HTRGTOL = PREPARE_TRGTOL(ALLOCATORS,KF_GP,KF_FS)
    IF (KF_FS > 0) THEN
      HFTDIR = PREPARE_FTDIR(ALLOCATORS(1),KF_FS)
      HTRLTOM_PACK = PREPARE_TRLTOM_PACK(ALLOCATORS(2), KF_FS)
      HTRLTOM = PREPARE_TRLTOM(ALLOCATORS(2), KF_FS)
      HTRLTOM_UNPACK = PREPARE_TRLTOM_UNPACK(ALLOCATORS(1), KF_FS)
      HLTDIR = PREPARE_LTDIR(ALLOCATORS(1), KF_FS, KF_UV)
    ENDIF

    CALL INSTANTIATE_ALLOCATOR(ALLOCATORS(1), GROWING_ALLOCATION)
    CALL INSTANTIATE_GAM_ALLOCATOR(ALLOCATORS(2), GROWING_GAM_ALLOCATION)

    ! from the PGP arrays to PREEL_REAL
    CALL TRGTOL(ALLOCATORS,HTRGTOL,PREEL_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,&
    ...
    IF (KF_FS > 0) THEN
      ...
      CALL GSTATS(1640,0)
      CALL FTDIR(ALLOCATORS(1),HFTDIR,PREEL_REAL,PREEL_COMPLEX,KF_FS)
      CALL GSTATS(1640,1)

      CALL GSTATS(153,0)

      CALL TRLTOM_PACK(ALLOCATORS(2),HTRLTOM_PACK,PREEL_COMPLEX,FOUBUF_IN,KF_FS)
      CALL TRLTOM(ALLOCATORS(2),HTRLTOM,FOUBUF_IN,FOUBUF,KF_FS)
      CALL TRLTOM_UNPACK(ALLOCATORS(1),HTRLTOM_UNPACK,FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV)
      CALL GSTATS(153,1)

      CALL LTDIR(ALLOCATORS(1),HLTDIR,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV,KF_SCALARS, &
