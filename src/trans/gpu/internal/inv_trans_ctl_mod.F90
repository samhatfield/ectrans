...
CONTAINS
  SUBROUTINE INV_TRANS_CTL(KF_UV_G,KF_SCALARS_G,KF_GP,KF_FS,KF_OUT_LT,&
    ...
    USE BUFFERED_ALLOCATOR_MOD, ONLY: BUFFERED_ALLOCATOR, MAKE_BUFFERED_ALLOCATOR, &
      &                               INSTANTIATE_ALLOCATOR, INSTANTIATE_GAM_ALLOCATOR
    ...
    TYPE(BUFFERED_ALLOCATOR) :: ALLOCATORS(2)
    ...
    ALLOCATORS(1) = MAKE_BUFFERED_ALLOCATOR()
    ALLOCATORS(2) = MAKE_BUFFERED_ALLOCATOR() ! Allocator for GPU-aware MPI buffers
    IF (KF_FS > 0) THEN
      HLTINV = PREPARE_LTINV(ALLOCATORS(1),KF_UV,KF_SCALARS,LVORGP,LDIVGP,LSCDERS)
      HTRMTOL_PACK = PREPARE_TRMTOL_PACK(ALLOCATORS(2),IF_LEG)
      HTRMTOL = PREPARE_TRMTOL(ALLOCATORS(2),IF_LEG)
      HTRMTOL_UNPACK = PREPARE_TRMTOL_UNPACK(ALLOCATORS(1),IF_FOURIER)
      HFSC = PREPARE_FSC(ALLOCATORS(1))
      HFTINV = PREPARE_FTINV(ALLOCATORS(1),IF_FOURIER)
    ENDIF
    HTRLTOG = PREPARE_TRLTOG(ALLOCATORS(2),IF_FOURIER,KF_GP)

    CALL INSTANTIATE_ALLOCATOR(ALLOCATORS(1), GROWING_ALLOCATION)
    CALL INSTANTIATE_GAM_ALLOCATOR(ALLOCATORS(2), GROWING_ALLOCATION)

    IF (KF_FS > 0) THEN
      ! Legendre transformations
      CALL GSTATS(102,0)
      CALL LTINV(ALLOCATORS(1),HLTINV,KF_UV,KF_SCALARS,&
      ...
      CALL GSTATS(152,0)
      CALL TRMTOL_PACK(ALLOCATORS(2),HTRMTOL_PACK,ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,FOUBUF_IN,IF_LEG)
      CALL TRMTOL(ALLOCATORS(2),HTRMTOL,FOUBUF_IN,FOUBUF,IF_LEG)
      CALL TRMTOL_UNPACK(ALLOCATORS(1),HTRMTOL_UNPACK,FOUBUF,PREEL_COMPLEX,IF_LEG,IF_FOURIER)
      CALL GSTATS(152,1)

      CALL GSTATS(107,0)
      ! compute NS derivatives
      CALL FSC(ALLOCATORS(1),HFSC,PREEL_COMPLEX, IF_FOURIER, KF_UV, KF_SCALARS, KUV_OFFSET, KSCALARS_OFFSET, &
          & KSCALARS_NSDER_OFFSET, KUV_EWDER_OFFSET, KSCALARS_EWDER_OFFSET)
      !Legendre transformations
      CALL FTINV(ALLOCATORS(1), HFTINV, PREEL_COMPLEX,PREEL_REAL,IF_FOURIER)
      CALL GSTATS(107,1)
    ENDIF

    ! Transposition into grid-point space
    CALL GSTATS(157,0)
    CALL TRLTOG(ALLOCATORS(2),HTRLTOG,PREEL_REAL,IF_FOURIER,KF_GP,KF_UV_G,KF_SCALARS_G,&
