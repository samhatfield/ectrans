...
!     ------------------------------------------------------------------
ASSOCIATE(D_NUMP=>D%NUMP, D_MYMS=>D%MYMS, ZEPSNM=>FG%ZEPSNM)

!*       1.    COMPUTE U V FROM VORTICITY AND DIVERGENCE.
!              ------------------------------------------

#ifdef OMPGPU
!$OMP TARGET DATA MAP(PRESENT,ALLOC:D,D_MYMS,D_NUMP,FG,ZEPSNM,PU,PV,PVOR,PDIV) MAP(TO:R)
#endif
#ifdef ACCGPU
!$ACC DATA &
!$ACC& PRESENT(D,D_MYMS,D_NUMP,R,R%NTMAX) &
...
DO KMLOC=1,D_NUMP
  DO J=1,2*KF_UV
    KM = D_MYMS(KMLOC)
    PU(J,R%NTMAX+4-KM,KMLOC) = 0.0_JPRBT
    PV(J,R%NTMAX+4-KM,KMLOC) = 0.0_JPRBT
  ENDDO
ENDDO
