...
CONTAINS
...
#ifdef OMPGPU
   !$OMP TARGET DATA MAP(ALLOC:FOUBUF_IN,PREEL_COMPLEX) MAP(TO:R,D,G)
#endif
#ifdef ACCGPU
    !$ACC DATA PRESENT(G,G%NMEN,D,D%NPNTGTB0,FOUBUF_IN,PREEL_COMPLEX,D%NSTAGTF,D%NDGL_FS,G%NLOEN, R,R%NSMAX) ASYNC(1)
#endif
    ...
    !$OMP& SHARED(D,R,KF_FS,OFFSET_VAR,G,PREEL_COMPLEX,FOUBUF_IN) MAP(TO:KF_FS,OFFSET_VAR)
#endif
#ifdef ACCGPU
      ...
      DO JM=0,R%NSMAX !(note that R%NSMAX <= G%NMEN(IGLG) for all IGLG)
        DO JF=1,KF_FS
          ...
          IF (JM <= G%NMEN(IGLG)) THEN
            IOFF_LAT = KF_FS*D%NSTAGTF(KGL)+(JF-1)*(D%NSTAGTF(KGL+1)-D%NSTAGTF(KGL))

            SCAL = 1._JPRBT/REAL(G%NLOEN(IGLG),JPRBT)
            ISTA  = 2_JPIB*D%NPNTGTB0(JM,KGL)*KF_FS
  ...
  SUBROUTINE TRLTOM_UNPACK(ALLOCATOR,HTRLTOM_UNPACK,FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV)
...
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(ALLOC:ZINPS,ZINPA,ZINPS0,ZINPA0,FOUBUF) MAP(TO:R,F,D,G)
    ...
    !$ACC& PRESENT(F,F%RW,F%RACTHE) &
    !$ACC& PRESENT(D,D%MYMS,D%NUMP,R,R%NDGNH,R%NDGL,G,G%NDGLU) &
    !$ACC& PRESENT(D%NPNTGTB1,D%OFFSETS_GEMM1,FOUBUF)
      ...
      DO JGL=1,R%NDGNH
        DO JF=1,KF_FS*2
          ...
          IF (JGL >= ISL) THEN
            !(DO JGL=ISL,R%NDGNH)
            IGLS = R%NDGL+1-JGL
            OFFSET1 = 2_JPIB*D%NPNTGTB1(KMLOC,JGL )*KF_FS
            OFFSET2 = 2_JPIB*D%NPNTGTB1(KMLOC,IGLS)*KF_FS
            PAIA = FOUBUF(OFFSET1+JF)-FOUBUF(OFFSET2+JF)
            PAIS = FOUBUF(OFFSET1+JF)+FOUBUF(OFFSET2+JF)
            IF (JF <= 4*KF_UV) THEN
                ! Multiply in case of velocity
              PAIA = PAIA*REAL(F%RACTHE(JGL),JPRBT)
              PAIS = PAIS*REAL(F%RACTHE(JGL),JPRBT)
            ENDIF
            IF (KM /= 0) THEN
              ZINPA(JF+(JGL-ISL)*IIN_STRIDES0+IIN_STRIDES0*D%OFFSETS_GEMM1(KMLOC))=PAIA*REAL(F%RW(JGL),JPRBT)
              ZINPS(JF+(JGL-ISL)*IIN_STRIDES0+IIN_STRIDES0*D%OFFSETS_GEMM1(KMLOC))=PAIS*REAL(F%RW(JGL),JPRBT)
            ELSEIF (MOD(JF-1,2) == 0) THEN
              ! every other field is sufficient because Im(KM=0) == 0
              ZINPA0((JF-1)/2+1+(JGL-1)*IIN0_STRIDES0)=PAIA*REAL(F%RW(JGL),JPRBT)
              ZINPS0((JF-1)/2+1+(JGL-1)*IIN0_STRIDES0)=PAIS*REAL(F%RW(JGL),JPRBT)
            ENDIF
          ENDIF
        ENDDO
      ENDDO
    ...
    !$OMP& SHARED(D,KF_FS,G,IIN_STRIDES0,ZINPA,ZINPS) MAP(TO:KF_FS)
#endif
#ifdef ACCGPU
    ...
    DO KMLOC=1,D%NUMP
      DO JF=1,KF_FS*2
          ...
          !$ACC LOOP SEQ
#endif
          DO JGL=G%NDGLU(KM),ALIGN(G%NDGLU(KM),A)-1
            IF (KM /= 0) THEN
              ZINPA(JF+JGL*IIN_STRIDES0+IIN_STRIDES0*D%OFFSETS_GEMM1(KMLOC))=0.0_JPRB
              ZINPS(JF+JGL*IIN_STRIDES0+IIN_STRIDES0*D%OFFSETS_GEMM1(KMLOC))=0.0_JPRB
            ENDIF
          ENDDO
      ENDDO
    END DO
