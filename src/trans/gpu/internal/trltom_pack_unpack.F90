...
CONTAINS
...
#ifdef OMPGPU
   !$OMP TARGET DATA MAP(PRESENT,ALLOC:G,G_NMEN,FOUBUF_IN,PREEL_COMPLEX,G_NLOEN) MAP(TO:R,D)
#endif
#ifdef ACCGPU
    !$ACC DATA PRESENT(G,G_NMEN,D,D%NPNTGTB0,FOUBUF_IN,PREEL_COMPLEX,D%NSTAGTF,D%NDGL_FS,G_NLOEN, R,R%NSMAX) ASYNC(1)
#endif
  ...
  SUBROUTINE TRLTOM_UNPACK(ALLOCATOR,HTRLTOM_UNPACK,FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV)
    ...
    !$OMP& MAP(PRESENT,ALLOC:G,G_NDGLU) &
    !$OMP& MAP(PRESENT,ALLOC:FOUBUF) MAP(TO:R,F,D)
    ...
    !$ACC& PRESENT(D,D%MYMS,D%NUMP,R,R%NDGNH,R%NDGL,G,G_NDGLU) &
    ...
    !$OMP& SHARED(D,KF_FS,G_NDGLU,IIN_STRIDES0,ZINPA,ZINPS) MAP(TO:KF_FS)
#endif
#ifdef ACCGPU
