...
CONTAINS
  ...
  SUBROUTINE TRLTOM_PACK(ALLOCATOR,HTRLTOM_PACK,PREEL_COMPLEX,FOUBUF_IN,KF_FS)
    ...
    ASSOCIATE(D_NSTAGTF=>D%NSTAGTF, D_NPNTGTB0=>D%NPNTGTB0, D_NPTRLS=>D%NPTRLS, &
            & D_NDGL_FS=>D%NDGL_FS, G_NMEN=>G%NMEN, G_NLOEN=>G%NLOEN)
   ...
   !$OMP& D_NDGL_FS,G_NLOEN) MAP(TO:R)
#endif
#ifdef ACCGPU
    !$ACC DATA PRESENT(G,G_NMEN,D,D_NPNTGTB0,FOUBUF_IN,PREEL_COMPLEX,D_NSTAGTF,D_NDGL_FS,G_NLOEN, R,R%NSMAX) ASYNC(1)
#endif
  ...
  SUBROUTINE TRLTOM_UNPACK(ALLOCATOR,HTRLTOM_UNPACK,FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV)
    ...
    ASSOCIATE(D_NUMP=>D%NUMP, D_MYMS=>D%MYMS, D_NPNTGTB1=>D%NPNTGTB1, &
            & D_OFFSETS_GEMM1=>D%OFFSETS_GEMM1, G_NDGLU=>G%NDGLU)
    CALL LEDIR_STRIDES(KF_FS,IIN_STRIDES0=IIN_STRIDES0,IIN_SIZE=IIN_SIZE,&
                       IIN0_STRIDES0=IIN0_STRIDES0,IIN0_SIZE=IIN0_SIZE)
    ...
    !$OMP& MAP(PRESENT,ALLOC:D,D_MYMS,D_NUMP,G,G_NDGLU) &
    !$OMP& MAP(PRESENT,ALLOC:D_NPNTGTB1,D_OFFSETS_GEMM1,FOUBUF) MAP(TO:R,F)
    ...
    !$ACC& PRESENT(D,D_MYMS,D_NUMP,R,R%NDGNH,R%NDGL,G,G_NDGLU) &
      ...
      DO JGL=1,R%NDGNH
        DO JF=1,KF_FS*2
          ...
          IF (JGL >= ISL) THEN
            ...
            ENDIF
            IF (KM /= 0) THEN
              ZINPA(JF+(JGL-ISL)*IIN_STRIDES0+IIN_STRIDES0*D_OFFSETS_GEMM1(KMLOC))=PAIA*REAL(F%RW(JGL),JPRBT)
              ZINPS(JF+(JGL-ISL)*IIN_STRIDES0+IIN_STRIDES0*D_OFFSETS_GEMM1(KMLOC))=PAIS*REAL(F%RW(JGL),JPRBT)
