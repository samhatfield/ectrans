...
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(TO:R,D,G) MAP(PRESENT,ALLOC:ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,FOUBUF_IN)
#endif
#ifdef ACCGPU
    !$ACC DATA PRESENT(D,D%MYMS,D%NPNTGTB1,D%NUMP,G,G%NDGLU,R,R%NDGNH,R%NDGL) &
...
#ifdef OMPGPU
!$OMP TARGET DATA MAP(PRESENT,ALLOC:FOUBUF,PREEL_COMPLEX) MAP(TO:D,G)
#endif
#ifdef ACCGPU
!$ACC DATA PRESENT(G,G%NLOEN,G%NMEN,D,D%NPNTGTB0,FOUBUF,PREEL_COMPLEX,D%NSTAGTF,D%NDGL_FS) ASYNC(1)
#endif
