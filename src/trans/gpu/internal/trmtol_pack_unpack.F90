  ...
  SUBROUTINE TRMTOL_PACK(ALLOCATOR,HTRMTOL_PACK,ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,FOUBUF_IN,KF_LEG)
...
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(PRESENT,ALLOC:G,G_NDGLU) MAP(TO:R,D) &
    !$OMP&            MAP(PRESENT,ALLOC:ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,FOUBUF_IN)
#endif
#ifdef ACCGPU
    !$ACC DATA PRESENT(D,D%MYMS,D%NPNTGTB1,D%NUMP,G,G_NDGLU,R,R%NDGNH,R%NDGL) &
    !$ACC&     PRESENT(ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,FOUBUF_IN,D%OFFSETS_GEMM1)
#endif
    ...
    DO KMLOC=1,D%NUMP
      DO JGL=1,R%NDGNH
        DO JK=1,2*KF_LEG
          KM = D%MYMS(KMLOC)
          ISL = R%NDGNH-G_NDGLU(KM)+1
          IF (JGL >= ISL) THEN
            !(DO JGL=ISL,R%NDGNH)
            IGLS = R%NDGL+1-JGL
            OFFSET1 = 2_JPIB*D%NPNTGTB1(KMLOC,JGL )*KF_LEG
            OFFSET2 = 2_JPIB*D%NPNTGTB1(KMLOC,IGLS)*KF_LEG

            IF(KM /= 0) THEN
              ZSOA = ZOUTS(JK+(JGL-ISL)*IOUT_STRIDES0+D%OFFSETS_GEMM1(KMLOC)*IOUT_STRIDES0)
              ZAOA = ZOUTA(JK+(JGL-ISL)*IOUT_STRIDES0+D%OFFSETS_GEMM1(KMLOC)*IOUT_STRIDES0)
...
ASSOCIATE(G_NLOEN=>G%NLOEN, G_NMEN=>G%NMEN)
...
#ifdef OMPGPU
!$OMP TARGET DATA MAP(PRESENT,ALLOC:G,G_NLOEN,G_NMEN,FOUBUF,PREEL_COMPLEX) MAP(TO:D)
#endif
#ifdef ACCGPU
!$ACC DATA PRESENT(G,G_NLOEN,G_NMEN,D,D%NPNTGTB0,FOUBUF,PREEL_COMPLEX,D%NSTAGTF,D%NDGL_FS) ASYNC(1)
#endif
    ...
    DO JM=0,ILOEN_MAX/2
      ...
      IF (JM <= G_NLOEN(IGLG)/2) THEN
        ...
        IF (JM <= G_NMEN(IGLG)) THEN
          ISTA  = 2_JPIB*D%NPNTGTB0(JM,KGL)*KF_CURRENT

          RET_REAL    = FOUBUF(ISTA+2*JF-1)
          RET_COMPLEX = FOUBUF(ISTA+2*JF  )
        ENDIF
        IOFF_LAT = 1_JPIB*KF_TOTAL*D%NSTAGTF(KGL)+(JF-1)*(D%NSTAGTF(KGL+1)-D%NSTAGTF(KGL))
