...
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(ALLOC:PGP) IF(PRESENT(PGP))
    !$OMP TARGET DATA MAP(ALLOC:PGPUV) IF(PRESENT(PGPUV))
    !$OMP TARGET DATA MAP(ALLOC:PGP2) IF(PRESENT(PGP2))
    !$OMP TARGET DATA MAP(ALLOC:PGP3A) IF(PRESENT(PGP3A))
    !$OMP TARGET DATA MAP(ALLOC:PGP3B) IF(PRESENT(PGP3B))

    !$OMP TARGET DATA MAP(TO:IIN_TO_SEND_BUFR) MAP(ALLOC:PREEL_REAL) IF(KF_FS > 0)
#endif
#ifdef ACCGPU
...
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(ALLOC:ZCOMBUFS) IF(ISEND_COUNTS > 0)
#endif
#ifdef ACCGPU
...
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(ALLOC:ZCOMBUFR) IF(IRECV_COUNTS > 0)
#endif
#ifdef ACCGPU
