...
CONTAINS
  FUNCTION PREPARE_TRLTOM(ALLOCATOR, KF_FS) RESULT(HTRLTOM)
    ...
    HTRLTOM%HPFBUF = RESERVE_GAM(ALLOCATOR, INT(D%NLENGT1B*2*KF_FS,KIND=C_SIZE_T)*C_SIZEOF(DUMMY))
  END FUNCTION
...
#define TRLTOM_DTYPE MPI_DOUBLE
    ...
    CALL ASSIGN_PTR(PFBUF, GET_ALLOCATION_GAM(ALLOCATOR, HTRLTOM%HPFBUF),&
        & 1_C_SIZE_T, INT(D%NLENGT1B*2*KF_FS,KIND=C_SIZE_T)*C_SIZEOF(PFBUF(1)))
