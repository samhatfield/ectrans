...
CONTAINS
  FUNCTION PREPARE_TRMTOL(ALLOCATOR, KF_LEG) RESULT(HTRMTOL)
    ...
    HTRMTOL%HPFBUF = RESERVE_GAM(ALLOCATOR, INT(D%NLENGT0B*2*KF_LEG,KIND=C_SIZE_T)*C_SIZEOF(DUMMY))
  END FUNCTION
...
#define TRMTOL_DTYPE MPI_DOUBLE
    ...
    CALL ASSIGN_PTR(PFBUF, GET_ALLOCATION_GAM(ALLOCATOR, HTRMTOL%HPFBUF),&
        & 1_C_SIZE_T, INT(D%NLENGT0B*2*KF_LEG,KIND=C_SIZE_T)*C_SIZEOF(PFBUF(1)))
