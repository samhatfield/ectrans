...
MODULE TRGTOL_MOD
  ...
  !$ACC DATA COPYIN(LLGP2,LLGP3A,LLGP3B,LLUV,IGPTRSEND) ASYNC(1)
  ...
  !$ACC DATA PRESENT(PGLAT) COPYIN(KPTRGP,INDOFF,KINDEX) ASYNC(1)
  ...
  IF(ISENDTOT(MYPROC) > 0 )THEN
    ...
    IGPTROFF(1)=0
    DO JBLK=2,NGPBLKS
      IGPTROFF(JBLK)=IGPTROFF(JBLK)+IGPTRSEND(2,JBLK,MYSETW)-IGPTRSEND(1,JBLK,MYSETW)+1
    ENDDO
  ...
  CALL GSTATS(1602,0)
    DO INS=1,INSEND
      ...
      IGPTROFF(1)=0
      DO JBLK=2,NGPBLKS
        IGPTROFF(JBLK)=IGPTROFF(JBLK)+IGPTRSEND(2,JBLK,ISETW)-IGPTRSEND(1,JBLK,ISETW)+1
      ENDDO
