...
MODULE TRGTOL_MOD
  ...
  INTEGER(KIND=JPIM) :: IFIRST, IFIRSTLAT, IGL, IGLL, ILAST,&
               ...
               &II,IBUFLENR,INRECV, IPROC,IFLDS, &
               &INSEND,INS,INR,IR, JKL, PBOUND, IERROR
  
  !     LOCAL LOGICAL SCALARS
  INTEGER(KIND=JPIM) :: IOFF
  INTEGER(KIND=JPIM) :: ZCOMBUFR_TO_PGLAT(D%NLENGTF),ZCOMBUFR_PROC_OFFSET(NPROC)
  INTEGER(KIND=JPIM) :: ISEND_FLD_TOTAL(NPRTRV),ISEND_FLD_CNT
  INTEGER(KIND=JPIM) :: IPOINTS_PER_WSET(NPRTRW), IWSET_POINT_OFFSET(NPRTRW+1), ILOCAL_LAT, ILAT_STRIP
  INTEGER(KIND=JPIM), ALLOCATABLE :: ICOMBUFS_OFFSET(:),ICOMBUFR_OFFSET(:)
  INTEGER(KIND=JPIM) :: SEND_OFFSET, RECV_OFFSET, IWPOINTS_COUNT, IWPOINTS_OFFSET
  !     INTEGER FUNCTIONS
  INTEGER(KIND=JPIM) :: INDOFFL
  ...
  IF(NPRTRV == 1) THEN
    ...
    ISEND_FLD_TOTAL(1) = KF_GP
  ELSE
    ISEND_FLD_TOTAL(:) = 0
    DO JFLD=1,KF_GP
      ISEND_FLD_TOTAL(KVSET(JFLD)) = ISEND_FLD_TOTAL(KVSET(JFLD)) + 1
    ENDDO
  ENDIF
  ...
  DO ILOCAL_LAT=D%NFRSTLAT(MY_REGION_NS),D%NLSTLAT(MY_REGION_NS)
    ...
    IPOINTS_PER_WSET(D%NPROCL(ILOCAL_LAT)) = &
        & IPOINTS_PER_WSET(D%NPROCL(ILOCAL_LAT))+D%NONL(ILAT_STRIP,MY_REGION_EW)
  ENDDO
  ! sum up offsets
  IWSET_POINT_OFFSET(1) = 0
  DO JROC=1,NPRTRW
    IWSET_POINT_OFFSET(JROC+1)=IWSET_POINT_OFFSET(JROC)+IPOINTS_PER_WSET(JROC)
  ENDDO
  DO JROC=1,NPROC
    CALL PE2SET(JROC,ISETA,ISETB,ISETW,ISETV)
    ! total send size is # points per field * # fields
    ISENDTOT(JROC) = IPOINTS_PER_WSET(ISETW)*ISEND_FLD_TOTAL(ISETV)
  ENDDO
  
  ! Prepare receiver arrays
  ZCOMBUFR_PROC_OFFSET(:) = 0
  DO JROC=1,NPROC
    ! Get new offset to my current ZCOMBUFR entry
    IF (JROC > 1 .AND. KF_FS > 0) THEN
      ZCOMBUFR_PROC_OFFSET(JROC) = ZCOMBUFR_PROC_OFFSET(JROC-1)+IRECVTOT(JROC-1)/KF_FS
    ELSEIF (JROC > 1) THEN
      ZCOMBUFR_PROC_OFFSET(JROC) = ZCOMBUFR_PROC_OFFSET(JROC-1)
    ENDIF
    ...
    DO JGL=IFIRSTLAT,ILASTLAT
      ...
      IGL  = JGL-D%NFRSTLAT(ISETA)+D%NPTRFRSTLAT(ISETA)
        ...
        ZCOMBUFR_TO_PGLAT(ZCOMBUFR_PROC_OFFSET(JROC)+IPOS) = D%NSTA(IGL,ISETB)+D%NSTAGTF(IGLL)+JL-1
        ENDDO
      ENDDO
  ...
  !$ACC DATA PRESENT(PGLAT) COPYIN(KPTRGP,ZCOMBUFR_PROC_OFFSET,ZCOMBUFR_TO_PGLAT) &
  ...
  IF(ISENDTOT(MYPROC) > 0 )THEN
    ...
    IWPOINTS_OFFSET = IWSET_POINT_OFFSET(MYSETW)
    IWPOINTS_COUNT = IPOINTS_PER_WSET(MYSETW)
  CALL GSTATS(1601,0)
  IF(PRESENT(PGP)) THEN
      !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS,IOFF) ASYNC(1)
      DO JL=1,IWPOINTS_COUNT
      DO JFLD=1,KF_FS
          JK = MOD(IWPOINTS_OFFSET+JL-1,NPROMA)+1
          JBLK = (IWPOINTS_OFFSET+JL-1)/NPROMA+1
          IFLD = IFLDA(JFLD)
          IPOS = ZCOMBUFR_PROC_OFFSET(MYPROC)+JL
          PGLAT(JFLD,ZCOMBUFR_TO_PGLAT(IPOS)) = PGP(JK,IFLDA(JFLD),JBLK)
      ENDDO
    ENDDO
  ELSE
      !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS,IOFF) ASYNC(1)
  ...
  ALLOCATE(ICOMBUFS_OFFSET(INSEND+1))
  ICOMBUFS_OFFSET(1) = 0
  DO JROC=1,INSEND
    ICOMBUFS_OFFSET(JROC+1) = ICOMBUFS_OFFSET(JROC) + ISENDTOT(JSEND(JROC))
  ENDDO
  ALLOCATE(ICOMBUFR_OFFSET(INRECV+1))
  ICOMBUFR_OFFSET(1) = 0
  DO JROC=1,INRECV
    ICOMBUFR_OFFSET(JROC+1) = ICOMBUFR_OFFSET(JROC) + IRECVTOT(JRECV(JROC))
  ENDDO

  ! Send loop.............................................................
  
  IF (INSEND > 0) ALLOCATE(ZCOMBUFS(ICOMBUFS_OFFSET(INSEND+1)))
  IF (INRECV > 0) ALLOCATE(ZCOMBUFR(ICOMBUFR_OFFSET(INRECV+1)))

  !$ACC DATA IF(INSEND > 0) CREATE(ZCOMBUFS) ASYNC(1)
  !$ACC DATA IF(INRECV > 0) CREATE(ZCOMBUFR) ASYNC(1)
  ...
  CALL GSTATS(1602,0)
    ...
    ISEND_FLD_CNT = ISEND_FLD_TOTAL(ISETV)
    SEND_OFFSET = ICOMBUFS_OFFSET(INS)
    ...
    !$ACC DATA COPYIN(IFLDA(1:ISEND_FLD_CNT)) ASYNC(1)
  
    IWPOINTS_OFFSET = IWSET_POINT_OFFSET(ISETW)
    IWPOINTS_COUNT = IPOINTS_PER_WSET(ISETW)
      IF(PRESENT(PGP)) THEN
      ...
      DO JL=1,IWPOINTS_COUNT
          ...
          JBLK = (IWPOINTS_OFFSET+JL-1)/NPROMA+1
          IFLD = IFLDA(JFLD)
          JI = (JFLD-1)*IWPOINTS_COUNT+JL
              ZCOMBUFS(SEND_OFFSET+JI) = PGP(JK,IFLDA(JFLD),JBLK)
          ENDDO
        ENDDO
      ELSE
      ...
      DO JL=1,IWPOINTS_COUNT
          ...
          JI = (JFLD-1)*IWPOINTS_COUNT+JL
                IF(IFLD < PGP_INDICES(PGP_INDICES_UV+1)) THEN
                  ...
                  PBOUND=UBOUND(PGPUV,2)
                  ! TODO we could certainly reshape PGPXX arrays and we would simplify this
                ZCOMBUFS(SEND_OFFSET+JI) = PGPUV(JK,MOD(IOFF,PBOUND)+1,IOFF/PBOUND+1,JBLK)
                ELSEIF(IFLD < PGP_INDICES(PGP_INDICES_GP2+1)) THEN
                  IOFF=IFLD-PGP_INDICES(PGP_INDICES_GP2)
                ZCOMBUFS(SEND_OFFSET+JI)  = PGP2(JK,IOFF+1,JBLK)
                ELSEIF(IFLD < PGP_INDICES(PGP_INDICES_GP3A+1)) THEN
                  IOFF=IFLD-PGP_INDICES(PGP_INDICES_GP3A)
                  PBOUND=UBOUND(PGP3A,2)
                ZCOMBUFS(SEND_OFFSET+JI) = PGP3A(JK,MOD(IOFF,PBOUND)+1,IOFF/PBOUND+1,JBLK)
                ELSEIF(IFLD < PGP_INDICES(PGP_INDICES_GP3B+1)) THEN
                  IOFF=IFLD-PGP_INDICES(PGP_INDICES_GP3B)
                  PBOUND=UBOUND(PGP3B,2)
                ZCOMBUFS(SEND_OFFSET+JI)= PGP3B(JK,MOD(IOFF,PBOUND)+1,IOFF/PBOUND+1,JBLK)
              ENDIF
        ENDDO
      ENDDO
      ENDIF
  ...
  DO INR=1,INRECV
    ...
    CALL MPI_IRECV(ZCOMBUFR(ICOMBUFR_OFFSET(INR)+1:ICOMBUFR_OFFSET(INR+1)),IRECVTOT(IRECV), &
      & TRGTOL_DTYPE,NPRCIDS(IRECV)-1,MTAGGL,MPL_COMM_OML(OML_MY_THREAD()),IREQ(IR),IERROR)
  ENDDO
  ...
  DO INR=1,INRECV
    ...
    INDOFFL = ZCOMBUFR_PROC_OFFSET(IRECV)
    RECV_OFFSET = ICOMBUFR_OFFSET(INR)
      !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(II) ASYNC(1)
      DO JFLD=1,KF_FS
        DO JL=1,ILEN
        II = ZCOMBUFR_TO_PGLAT(INDOFFL+JL)
        PGLAT(JFLD,II) = ZCOMBUFR(RECV_OFFSET+JL+(JFLD-1)*ILEN)
        ENDDO
      ENDDO
  ENDDO
  !$ACC WAIT(1)
  ...
  IF (INSEND > 0) DEALLOCATE(ZCOMBUFS)
  IF (INRECV > 0) DEALLOCATE(ZCOMBUFR)
