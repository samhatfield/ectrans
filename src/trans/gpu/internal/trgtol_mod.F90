...
MODULE TRGTOL_MOD
  ...
  END TYPE
CONTAINS
  FUNCTION PREPARE_TRGTOL(ALLOCATORS,KF_GP,KF_FS) RESULT(HTRGTOL)
    ...
    TYPE(BUFFERED_ALLOCATOR), INTENT(INOUT) :: ALLOCATORS(2)
    ...
    HTRGTOL%HCOMBUFS_COMBUFR = RESERVE(ALLOCATORS(2), NELEM)
    
    HTRGTOL%REEL_START = 1
    NELEM = ALIGN(INT(KF_FS*D%NLENGTF,KIND=C_SIZE_T)*C_SIZEOF(DUMMY),128) ! PREEL_REAL
    HTRGTOL%HREEL = RESERVE(ALLOCATORS(1), NELEM) 
  END FUNCTION PREPARE_TRGTOL

  SUBROUTINE TRGTOL(ALLOCATORS,HTRGTOL,PREEL_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,&
    ...
    TYPE(BUFFERED_ALLOCATOR), INTENT(IN) :: ALLOCATORS(2)
...
#define TRGTOL_DTYPE MPI_DOUBLE
    ...
    block
    CALL ASSIGN_PTR(PREEL_REAL, GET_ALLOCATION(ALLOCATORS(1), HTRGTOL%HREEL),&
    ...
    IF (ISEND_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFS, GET_ALLOCATION(ALLOCATORS(2), HTRGTOL%HCOMBUFS_COMBUFR),&
    ...
    IF (IRECV_COUNTS > 0) THEN
      CALL ASSIGN_PTR(ZCOMBUFR, GET_ALLOCATION(ALLOCATORS(2), HTRGTOL%HCOMBUFS_COMBUFR),&
