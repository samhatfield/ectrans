...
MODULE TRGTOL_MOD
  ...
  INTEGER(KIND=JPIM) :: IFIRST, IFIRSTLAT, IGL, IGLL, ILAST,&
               ...
               &INSEND,INS,INR,IR, JKL, JK_MAX, PBOUND, IERROR
  ...
  INTEGER(KIND=JPIM) :: SEND_OFFSET, RECV_OFFSET
  !     INTEGER FUNCTIONS
  ...
  !$ACC&     COPYIN(IGPTRSEND,PGP_INDICES) ASYNC(1)
  ...
  IF(ISENDTOT(MYPROC) > 0 )THEN
    ...
    IGPTROFF(1)=0
    DO JBLK=2,NGPBLKS
      IGPTROFF(JBLK)=IGPTROFF(JBLK-1)+IGPTRSEND(2,JBLK-1,MYSETW)-IGPTRSEND(1,JBLK-1,MYSETW)+1
    ENDDO
  ...
  CALL GSTATS(1601,0)
  IF(PRESENT(PGP)) THEN
    !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK) ASYNC(1)
    DO JBLK=1,NGPBLKS
      DO JFLD=1,KF_FS
        DO JKL=1, JK_MAX
    ...
    ENDDO
  ELSE
      !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK,IOFF,PBOUND) ASYNC(1)
    DO JBLK=1,NGPBLKS
      DO JFLD=1,KF_FS
        DO JKL=1, JK_MAX
          IFIRST = IGPTRSEND(1,JBLK,MYSETW)
  ...
  CALL GSTATS(1602,0)
    ...
    SEND_OFFSET = ICOMBUFS_OFFSET(INS)
      ...
      IF(PRESENT(PGP)) THEN
        !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(JK,JI,IFIRST,ILAST) ASYNC(1)
        DO JBLK=1,NGPBLKS
        ...
        ENDDO
      ELSE
        !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(JK,JI,IFLD,IFIRST,ILAST) ASYNC(1)
          DO JBLK=1,NGPBLKS
          DO JFLD=1,ISEND_FLD_CNT
            DO JKL=1, JK_MAX
              IFLD=IFLDA(JFLD)
