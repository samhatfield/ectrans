...
#define TRGTOL_DTYPE MPI_REAL8
    ...
    ENDIF
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(TO:PGP) IF(PRESENT(PGP) .AND. KF_GP > 0)
    !$OMP TARGET DATA MAP(TO:PGPUV) IF(PRESENT(PGPUV))
    !$OMP TARGET DATA MAP(TO:PGP2) IF(PRESENT(PGP2))
    !$OMP TARGET DATA MAP(TO:PGP3A) IF(PRESENT(PGP3A))
    !$OMP TARGET DATA MAP(TO:PGP3B) IF(PRESENT(PGP3B))
#endif
#ifdef ACCGPU
