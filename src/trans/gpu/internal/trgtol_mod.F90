...
MODULE TRGTOL_MOD
  ...
  INTEGER(KIND=JPIM) :: IFLDA(KF_GP),JJ,JI,IFLDT
  ...
  !$ACC&     COPYIN(LLGP2,LLGP3A,LLGP3B,LLUV,IGPTRSEND) ASYNC(1)
  !$ACC DATA IF(PRESENT(PGP))   PRESENT(PGP)   ASYNC(1)
  !$ACC DATA IF(PRESENT(PGPUV)) PRESENT(PGPUV) COPYIN(IUVLEVS,IUVPARS) ASYNC(1)
  !$ACC DATA IF(PRESENT(PGP2))  PRESENT(PGP2)  COPYIN(IGP2PARS) ASYNC(1)
  !$ACC DATA IF(PRESENT(PGP3A)) PRESENT(PGP3A) COPYIN(IGP3ALEVS,IGP3APARS) ASYNC(1)
  !$ACC DATA IF(PRESENT(PGP3B)) PRESENT(PGP3B) COPYIN(IGP3BLEVS,IGP3BPARS) ASYNC(1)
  ...
  CALL GSTATS(1602,0)
    DO INS=1,INSEND
      ...
      !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(JK,JI,IFLDT,IFIRST,ILAST) COPYIN(IGPTROFF,IFLDA) ASYNC(1)
