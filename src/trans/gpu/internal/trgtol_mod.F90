...
MODULE TRGTOL_MOD
  ...
  !     INTEGER FUNCTIONS
  INTEGER(KIND=JPIM) :: IRECVCOUNT,ISENDCOUNT,INDOFFL
  ...
  IF (IBUFLENS > 0) ALLOCATE(ZCOMBUFS(ICOMBUFS_OFFSET(INSEND+1)))
  IF (IBUFLENR > 0) ALLOCATE(ZCOMBUFR(ICOMBUFR_OFFSET(INRECV+1)))

  !$ACC DATA IF(IBUFLENS > 0) CREATE(ZCOMBUFS) ASYNC(1)
  !$ACC DATA IF(IBUFLENR > 0) CREATE(ZCOMBUFR) ASYNC(1)
