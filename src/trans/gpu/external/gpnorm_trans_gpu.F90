...
IF( IF_FS > 0 )THEN
...
#ifdef ACCGPU
  !$ACC DATA &
  !$ACC& COPY(D,D%NSTAGTF,D%NPTRLS,G%NLOEN,F,F%RW) &
  !$ACC& PRESENT(ZGTF,ZAVE,ZMINGL,ZMAXGL,ZMINGPN,ZMAXGPN)
  !$ACC KERNELS
#endif
#ifdef OMPGPU
  !$OMP TARGET DATA MAP(TO:F,D,D%NSTAGTF,D%NPTRLS,G%NLOEN) MAP(ALLOC:ZGTF) &
  !$OMP&          MAP(TO:ZAVE,ZMINGL,ZMAXGL,ZMINGPN,ZMAXGPN)
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO
#endif
  DO JF=1,IF_FS
    V = ZGTF(IF_FS*D%NSTAGTF(1)+(JF-1)*(D%NSTAGTF(2)-D%NSTAGTF(1)))
  ...
  DO JGL=IBEG,IEND
    IGL = D%NPTRLS(MYSETW) + JGL - 1
    DO JF=1,IF_FS
      ZAVE(JF,JGL)=ZAVE(JF,JGL)*F%RW(IGL)/G%NLOEN(IGL)
      !write(iunit,*) 'aver inside ',JF,IF_FS,IGL,ZAVE(JF,JGL), F%RW(IGL), G%NLOEN(IGL),ZMINGPN(JF),ZMAXGPN(JF)
    ENDDO
  ENDDO
#ifdef ACCGPU
  !$ACC END KERNELS
