...
IF( IF_FS > 0 )THEN
...
#ifdef OMPGPU
  !$OMP TARGET DATA MAP(TO:F,D,G)
#endif
#ifdef ACCGPU
  !$ACC DATA PRESENT(F,F_RW,D,D%NSTAGTF,D%NPTRLS,G%NLOEN)
#endif
...
#endif
  DO JF=1,IF_FS
    V = PREEL_REAL(IF_FS*D%NSTAGTF(1)+(JF-1)*(D%NSTAGTF(2)-D%NSTAGTF(1)))
  ...
  DO JGL=IBEG,IEND
    IGL = D%NPTRLS(MYSETW) + JGL - 1
    DO JF=1,IF_FS
      ZAVE(JF,JGL)=ZAVE(JF,JGL)*F_RW(IGL)/G%NLOEN(IGL)
      !write(iunit,*) 'aver inside ',JF,IF_FS,IGL,ZAVE(JF,JGL), F_RW(IGL), G%NLOEN(IGL),ZMINGPN(JF),ZMAXGPN(JF)
    ENDDO
  ENDDO
#ifdef ACCGPU
  !$ACC END KERNELS
