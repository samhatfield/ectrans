...
SUBROUTINE TRGTOL_COMM_SEND(PGLAT,PCOMBUFS,PCOMBUFR,IOFFSEND,IOFFRECV,KF_FS,KF_GP,KF_SCALARS_G,KVSET,&
 & KSENDCOUNT,KRECVCOUNT,KNSEND,KNRECV,KSENDTOT,KRECVTOT,KSEND,KRECV,KINDEX,KNDOFF,KGPTRSEND,&
 &  KREQ_RECV, &
...
SUBROUTINE TRGTOL_COMM_RECV(PGLAT, PCOMBUFR, IOFFRECV, IRECV_FLD_END,KF_FS, KRECVCOUNT, KNRECV, KRECVTOT, &
...
!  !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JL,II,JFLD)
  DO JL = 1, ILEN
    ...
    DO JFLD = 1, IRECV_FLD_END(JNR)
      PGLAT(JFLD,II) = PCOMBUFR(IOFFRECV+JL+(JFLD-1)*ILEN-1,JNR)
    ENDDO
  ENDDO
!  !$OMP END PARALLEL DO
ENDDO
