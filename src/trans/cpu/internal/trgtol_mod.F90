...
SUBROUTINE TRGTOL(PGLAT,KF_FS,KF_GP,KF_SCALARS_G,KVSET,&
...
INTEGER(KIND=JPIM) :: IGPTRSEND(2,NPROMA*NGPBLKS,NPRTRNS)
...
INTEGER(KIND=JPIM), INTENT(OUT) :: KGPTRSEND(2,NPROMA*NGPBLKS,NPRTRNS)
...
CALL INIGPTR(KGPTRSEND,IGPTRRECV,1,NPROMA*NGPBLKS)
...
INTEGER(KIND=JPIM), INTENT(IN) :: KGPTRSEND(2,NPROMA*NGPBLKS,NPRTRNS)
REAL(KIND=JPRB),OPTIONAL,INTENT(IN)     :: PGP(:,:)
REAL(KIND=JPRB),OPTIONAL,INTENT(IN)     :: PGPUV(:,:,:)
REAL(KIND=JPRB),OPTIONAL,INTENT(IN)     :: PGP3A(:,:,:)
REAL(KIND=JPRB),OPTIONAL,INTENT(IN)     :: PGP3B(:,:,:)
...
USE TPM_TRANS       ,ONLY : LDIVGP, LGPNORM, LSCDERS, LUVDER, LVORGP, NPROMA, NGPBLKS
...
INTEGER(KIND=JPIM) :: IJPOS(NPROMA*NGPBLKS,KNSEND)
...
INTEGER(KIND=JPIM) :: IFIRST, ILEN, IPOS, ISETA, ISETB, IRECV, ISETV
INTEGER(KIND=JPIM) :: ISEND, ITAG, JBLK, JFLD, JL, IFLD, II, IFLDS, INS, INR
...
INTEGER(KIND=JPIM) :: IGPTROFF(NPROMA*NGPBLKS)
...
!$OMP PARALLEL DO SCHEDULE(DYNAMIC) PRIVATE(JFLD64,JBLK,IFLD,IPOS,IFIRST)
#else
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD64,JBLK,IFLD,IPOS,IFIRST)
#endif
  DO JBLK=1,NPROMA*NGPBLKS
    IFIRST = KGPTRSEND(1,JBLK,MYSETW)
    IF(IFIRST > 0) THEN
      IPOS = KNDOFF(MYPROC)+IGPTROFF(JBLK)+1
      ! Address PGLAT over 64 bits because its size may exceed 2 GB for big data and
      ! small number of tasks.
      IF(LLPGPONLY) THEN
        DO JFLD64=1,IFLDS
          IFLD = IFLDOFF(JFLD64)
          PGLAT(KINDEX(IPOS),JFLD64) = PGP(JBLK,IFLD)
        ENDDO
      ELSE
        DO JFLD64=1,IFLDS
          IFLD = IFLDOFF(JFLD64)
          IF(LLUV(IFLD)) THEN
            PGLAT(KINDEX(IPOS),JFLD64) = PGPUV(JBLK,IUVLEVS(IFLD),IUVPARS(IFLD))
          ELSEIF(LLGP2(IFLD)) THEN
            PGLAT(KINDEX(IPOS),JFLD64) = PGP2(JBLK,IGP2PARS(IFLD))
          ELSEIF(LLGP3A(IFLD)) THEN
            PGLAT(KINDEX(IPOS),JFLD64) = PGP3A(JBLK,IGP3ALEVS(IFLD),IGP3APARS(IFLD))
          ELSEIF(LLGP3B(IFLD)) THEN
            PGLAT(KINDEX(IPOS),JFLD64) = PGP3B(JBLK,IGP3BLEVS(IFLD),IGP3BPARS(IFLD))
          ELSE
...
!$OMP PARALLEL PRIVATE(JBLK,IFIRST,INS,ISEND,ISETA,ISETB,ISETV,IFLD,IPOS,JFLD)
...
DO INS=1,KNSEND
  ...
  ! I couldn't find a way to multithread this loop without triggering a segfault
  !!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JBLK,IFIRST,IFLDT,JJ,JI)
  DO JJ=ISEND_FLD_START,ISEND_FLD_END
    IFLDT=IFLDA(JJ,INS)
    DO JBLK=1,NPROMA*NGPBLKS
      IFIRST = KGPTRSEND(1,JBLK,ISETW(INS))
      IF(IFIRST > 0) THEN
        JI=(JJ-ISEND_FLD_START)*IPOS+IJPOS(JBLK,INS)+1
        IF(LLPGPONLY) THEN
          PCOMBUFS(JI,INS) = PGP(JBLK,IFLDT)
        ELSEIF(LLUV(IFLDT)) THEN
          PCOMBUFS(JI,INS) = PGPUV(JBLK,IUVLEVS(IFLDT),IUVPARS(IFLDT))
        ELSEIF(LLGP2(IFLDT)) THEN
          PCOMBUFS(JI,INS) = PGP2(JBLK,IGP2PARS(IFLDT))
        ELSEIF(LLGP3A(IFLDT)) THEN
          PCOMBUFS(JI,INS) = PGP3A(JBLK,IGP3ALEVS(IFLDT),IGP3APARS(IFLDT))
        ELSEIF(LLGP3B(IFLDT)) THEN
          PCOMBUFS(JI,INS) = PGP3B(JBLK,IGP3BLEVS(IFLDT),IGP3BPARS(IFLDT))
        ENDIF
      ENDIF
    ENDDO
  ENDDO
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JL,II,JFLD)
  DO JFLD=IRECV_FLD_START,IRECV_FLD_END
    DO JL=1,ILEN
      ...
      PGLAT(II,JFLD) = PCOMBUFR(JL+(JFLD-IRECV_FLD_START)*ILEN,INR)
    ENDDO
  ENDDO
!$OMP END PARALLEL DO
ENDDO
