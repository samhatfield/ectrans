...
REAL(KIND=JPRB), INTENT(INOUT) :: PCOMBUFS(KSENDCOUNT,KNSEND)
REAL(KIND=JPRB), INTENT(INOUT) :: PCOMBUFR(KRECVCOUNT,KNRECV)
...
DO INR=1,KNRECV
  IRECV=KRECV(INR)
  CALL MPL_RECV(PCOMBUFR(KRECVTOT(IRECV),INR), &
...
DO INS=1,KNSEND
  ISEND=KSEND(INS)
  CALL MPL_SEND(PCOMBUFS(KSENDTOT(ISEND),INS),KDEST=NPRCIDS(ISEND), &
...
SUBROUTINE TRGTOL_COMM_RECV(PGLAT, PCOMBUFR, IRECV_FLD_END,KF_FS, KRECVCOUNT, KNRECV, KRECVTOT, KRECV, KINDEX, &
     &                      KNDOFF, MYOFFRECV, KREQ_RECV)
