...
MODULE INIGPTR_MOD
CONTAINS
SUBROUTINE INIGPTR(KGPTRSEND,KGPTRRECV,KPROMA,KGPBLKS)
...
INTEGER(KIND=JPIM),INTENT(OUT) :: KGPTRSEND(2,KGPBLKS,NPRTRNS)
...
DO JGL=1,D%NDGL_GP
  ...
  DO JBL=IFIRST,ILAST
    IF(IROF == KPROMA) THEN
      ...
      IF(IBLOCK < KGPBLKS) IBLOCK=IBLOCK+1
      IROF=0
      IBFIRST=1
    ENDIF
    IROF=IROF+1
