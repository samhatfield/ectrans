  ...
  DO WHILE (NDONE < IBLKS)
    ...
    IF (COMM_COMPL) THEN
      ...
      DO WHILE (ASSOCIATED(IB))
        SELECT TYPE (THISBATCH => IB%VALUE)
        TYPE IS (BATCH)
          IF (THISBATCH%STATUS == STAT_PENDING .AND. (THISBATCH%STAGE == 1 .OR. THISBATCH%NBLK == LAST_SUBMITTED + 1)) THEN
            CALL THISBATCH%START_COMM(PGP, IREQ_RECV(:,THISBATCH%NBLK), BGTF, ZCOMBUFS, ZCOMBUFR)
            NCOMM_STARTED = NCOMM_STARTED + 1
            IF (THISBATCH%STAGE == 2) LAST_SUBMITTED = LAST_SUBMITTED + 1
            EXIT
          END IF
          IB => IB%NEXT
        END SELECT
      ...
      IF (COMPLETE_COMM_BATCH%STAGE == STAGE_FINAL) THEN
        ...
        NACTIVE = NACTIVE - 1
        NDONE = NDONE + 1
      ELSEIF (NCOMM_STARTED < MAX_COMMS .AND. COMPLETE_COMM_BATCH%NBLK == LAST_SUBMITTED + 1) THEN
