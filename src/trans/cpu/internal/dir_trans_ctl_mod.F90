...
IF (NPROMATR > 0 .AND. KF_GP > NPROMATR) THEN
  ...
  KRECVCOUNT=MAXVAL(KRECVTOT)
  KSENDCOUNT=MAXVAL(KSENDTOT)
  DEALLOCATE(KRECVTOT)
  ...
  CALL ACTIVATE(JBLK, KF_GP, KF_SCALARS_G, KF_UV_G, KVSETUV, KVSETSC, PGP, IOFFSEND, IOFFRECV, IOFFGTF, KSENDCOUNT, KRECVCOUNT)
    ...
    DO WHILE (ASSOCIATED(IB))
      ...
      TYPE IS (BATCH)
        IF (THISBATCH%COMM_COMPLETE(IREQ_RECV(:,THISBATCH%NBLK))) THEN
          CALL THISBATCH%FINISH_COMM(BGTF, IRECV_REQ(:,THISBATCH%NBLK), ZCOMBUFR, PSPVOR, PSPDIV, PSPSCALAR)
...
SUBROUTINE ACTIVATE(N, KF_GP, KF_SCALARS_G, KF_UV_G, KVSETUV, KVSETSC, PGP, IOFFSEND, IOFFRECV, IOFFGTF, SENDCNTMAX, RECVCNTMAX)
  INTEGER,                      INTENT(IN)    :: N
  INTEGER(KIND=JPIM),           INTENT(IN)    :: KF_GP
  INTEGER(KIND=JPIM),           INTENT(IN)    :: KF_SCALARS_G
  INTEGER(KIND=JPIM),           INTENT(IN)    :: KF_UV_G
