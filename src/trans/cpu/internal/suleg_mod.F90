...
REAL(KIND=JPRD) :: ZANM
REAL(KIND=JPRD) :: ZFNN
REAL(KIND=JPRD) :: ZPI, ZINC, ZOFF, ZTEMP, ZORIG, ZTHETA, ZCOS, ZFACT
...
ELSE
  ...
  DO JGL=1,INN/2
    ZTHETA = REAL(JGL,JPRD)*ZINC
    ZLRMUZ(JGL) = COS(ZTHETA)
    ZFACT=0._JPRD
    DO JNP=1,INN-1,2
      ZFACT=ZFACT+SIN(REAL(JNP,JPRD)*ZTHETA)/REAL(JNP,JPRD)
    ENDDO
    ZW(JGL) = (4._JPRD*SIN(ZTHETA)/REAL(INN,JPRD))*ZFACT
  ENDDO
  ! shift/duplicate equator
  ZLRMUZ(INN/2_JPIM+1_JPIM) = ZLRMUZ(INN/2_JPIM)
  ZW(INN/2_JPIM+1_JPIM) = ZW(INN/2_JPIM)
  DO JGL=INN/2_JPIM+1_JPIM,INN-1
    ZTHETA = REAL(JGL,JPRD)*ZINC
    ZLRMUZ(JGL+1) = COS(ZTHETA)
    ZFACT=0._JPRD
    DO JNP=1,INN-1,2
      ZFACT=ZFACT+SIN(REAL(JNP,JPRD)*ZTHETA)/REAL(JNP,JPRD)
    ENDDO
    ZW(JGL+1) = (4._JPRD*SIN(ZTHETA)/REAL(INN,JPRD))*ZFACT
  ENDDO
