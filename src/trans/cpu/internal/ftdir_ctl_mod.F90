...
USE TPM_GEN   ,ONLY : NSTACK_MEMORY_TR, NOUT
USE TPM_TRANS       ,ONLY : FOUBUF_IN, NGPBLKS, NPROMA
...
REAL(KIND=JPRB), TARGET :: PIN_STACK(D%NLENGTF,KF_FS)
REAL(KIND=JPRB), TARGET, ALLOCATABLE :: PIN_HEAP(:,:)
REAL(KIND=JPRB), POINTER, CONTIGUOUS :: PIN(:,:)
...
REAL(KIND=JPRB), ALLOCATABLE :: PGP_TMP(:,:)
REAL(KIND=JPRB), ALLOCATABLE :: PGPUV_TMP(:,:,:)
REAL(KIND=JPRB), ALLOCATABLE :: PGP3A_TMP(:,:,:)
REAL(KIND=JPRB), ALLOCATABLE :: PGP3B_TMP(:,:,:)
REAL(KIND=JPRB), ALLOCATABLE :: PGP2_TMP(:,:)

INTEGER(KIND=JPIM) :: JBLK, KPROMA

IF (PRESENT(PGP))   ALLOCATE(PGP_TMP(NPROMA*NGPBLKS,SIZE(PGP,2)))
IF (PRESENT(PGPUV)) ALLOCATE(PGPUV_TMP(NPROMA*NGPBLKS,SIZE(PGPUV,2),SIZE(PGPUV,3)))
IF (PRESENT(PGP3A)) ALLOCATE(PGP3A_TMP(NPROMA*NGPBLKS,SIZE(PGP3A,2),SIZE(PGP3A,3)))
IF (PRESENT(PGP3B)) ALLOCATE(PGP3B_TMP(NPROMA*NGPBLKS,SIZE(PGP3B,2),SIZE(PGP3B,3)))
IF (PRESENT(PGP2))  ALLOCATE(PGP2_TMP(NPROMA*NGPBLKS,SIZE(PGP2,2)))
...
ELSE
  ...
  ALLOCATE(PIN_HEAP(D%NLENGTF,KF_FS))
...
IF (PRESENT(PGP)) THEN
  DO JBLK = 1, NGPBLKS
    DO KPROMA = 1, NPROMA
      PGP_TMP((JBLK-1)*NPROMA + KPROMA,:) = PGP(KPROMA,:,JBLK)
      ! or PGP_TMP((KPROMA-1)*NGPBLKS + JBLK)
    ENDDO
  ENDDO
ENDIF
IF (PRESENT(PGPUV)) THEN
  DO JBLK = 1, NGPBLKS
    DO KPROMA = 1, NPROMA
      PGPUV_TMP((JBLK-1)*NPROMA + KPROMA,:,:) = PGPUV(KPROMA,:,:,JBLK)
    ENDDO
  ENDDO
ENDIF
IF (PRESENT(PGP3A)) THEN
  DO JBLK = 1, NGPBLKS
    DO KPROMA = 1, NPROMA
      PGP3A_TMP((JBLK-1)*NPROMA + KPROMA,:,:) = PGP3A(KPROMA,:,:,JBLK)
    ENDDO
  ENDDO
ENDIF
IF (PRESENT(PGP3B)) THEN
  DO JBLK = 1, NGPBLKS
    DO KPROMA = 1, NPROMA
      PGP3B_TMP((JBLK-1)*NPROMA + KPROMA,:,:) = PGP3B(KPROMA,:,:,JBLK)
    ENDDO
  ENDDO
ENDIF
IF (PRESENT(PGP2)) THEN
  DO JBLK = 1, NGPBLKS
    DO KPROMA = 1, NPROMA
      PGP2_TMP((JBLK-1)*NPROMA + KPROMA,:) = PGP2(KPROMA,:,JBLK)
    ENDDO
  ENDDO
ENDIF

WRITE(NOUT,*) "PACKING DONE"
FLUSH(NOUT)

CALL GSTATS(158,0)
CALL TRGTOL(PIN,KF_FS,KF_GP,KF_SCALARS_G,IVSET,&
 &PGP_TMP,PGPUV_TMP,PGP3A_TMP,PGP3B_TMP,PGP2_TMP)
CALL GSTATS(158,1)
CALL GSTATS(106,0)
