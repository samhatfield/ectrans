...
!    ENDDO
  ...
  FUNCTION COMM_COMPLETE(THIS, IREQ_RECV)
    USE MPI,     ONLY: MPI_STATUS_SIZE, MPI_TESTALL,MPI_STATUSES_IGNORE
...
!      CALL MPI_TESTALL(THIS%NNRECV, IREQ_RECV(1:THIS%NNRECV), COMM_COMPLETE, ISTATS, IERROR)
    CALL MPI_TESTALL(THIS%NNRECV, IREQ_RECV(1:THIS%NNRECV), COMM_COMPLETE, MPI_STATUSES_IGNORE, &
         &           IERROR)
!    ENDDO
  ...
  SUBROUTINE EXECUTE(THIS, PGTF, IREQ_RECV, PCOMBUFR, PSPVOR, PSPDIV, PSPSCALAR)
    ...
    CALL LTDIR_CTL(THIS%IOFFGTF,THIS%NF_FS, THIS%NF_UV, THIS%NF_SCALARS, &
