  ...
  FUNCTION BATCH_CONSTRUCTOR(KBLK, KF_GP, KF_SCALARS_G, KF_UV_G, KVSETUV, KVSETSC, IOFFSEND, &
       &      IOFFRECV, IOFFGTF, SENDCNTMAX, RECVCNTMAX) RESULT(THIS)
        ...
        DO INS=1,THIS%NNSEND
           ...
           CALL MPI_ISEND(ISEND_FLD_END(INS),1,MPI_INTEGER,ISEND,KBLK,MPI_COMM_WORLD,IREQ_SEND(INS),IERR)
        ENDDO
        
        DO INR=1,THIS%NNRECV
           IRECV = THIS%NRECV(INR)
           CALL MPI_RECV(THIS%IRECV_FLD_END(INR),1,MPI_INTEGER,IRECV,KBLK,MPI_COMM_WORLD,MPI_STATUS_IGNORE,IERR)
        ENDDO
