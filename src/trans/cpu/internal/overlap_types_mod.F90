...
CONTAINS
  ...
  FUNCTION BATCH_CONSTRUCTOR(KBLK, KF_GP, KF_SCALARS_G, KF_UV_G, KVSETUV, KVSETSC, IOFFSEND, &
    ...
    CALL FIELD_SPLIT(KBLK, KF_GP, KF_UV_G, KVSETUV, KVSETSC, NSTUV_G, &
    ...
    DO JFLD = 1, NF_UV_G
      THIS%NPTRGP(JFLD) = JFLD
      THIS%NPTRGP(JFLD+NF_UV_G) = KF_UV_G + JFLD
    ENDDO
    DO JFLD = 1, THIS%NF_SCALARS_G
      THIS%NPTRGP(JFLD+2*NF_UV_G) = 2*KF_UV_G + JFLD
    ENDDO
    ...
    IST = 1
    IF (NF_UV_G > 0) THEN
      THIS%NVSET(IST:IST+NF_UV_G-1) = KVSETUV(NSTUV_G:NENUV_G)
      IST = IST + NF_UV_G
      THIS%NVSET(IST:IST+NF_UV_G-1) = KVSETUV(NSTUV_G:NENUV_G)
      IST = IST + NF_UV_G
    ENDIF
    IF (THIS%NF_SCALARS_G > 0) THEN
      THIS%NVSET(IST:IST+THIS%NF_SCALARS_G-1) = KVSETSC(NSTSC_G:NENSC_G)
  ...
  SUBROUTINE START_COMM(THIS, PGP, IREQ_RECV,PIN,PCOMBUFS,PCOMBUFR)
