...
!*       1. ANTISYMMETRIC PART.
  ...
  IF(ILA <= ITHRESHOLD .OR. .NOT.S%LUSEFLT) THEN
    ...
    IF (LLDOUBLE) THEN
       CALL GEMM('N','N',KIFC,ILA,KDGLU,1.0_JPRD,ZB,KIFC,S%FA(KMLOC)%RPNMA,KDGLU,&
            & 0._JPRD,ZCA,KIFC)
    ELSE
       IF(KM>=1)THEN ! DGEM for the mean to improve mass conservation
          CALL GEMM('N','N',KIFC,ILA,KDGLU,1.0_JPRM,ZB,KIFC,S%FA(KMLOC)%RPNMA,KDGLU,&
               & 0._JPRM,ZCA,KIFC)
       ELSE
          ...
          DO I3=1,I1
             ...
             END DO
          END DO
          CALL GEMM('N','N',KIFC,ILA,KDGLU,1.0_JPRD,ZB_D,KIFC,ZRPNMA,KDGLU,&
               & 0._JPRD,ZCA_D,KIFC)
          DO J=1,ILA
             IFLD=0
             DO JK=1,KFC,ISKIP
...
!*       1.3      SYMMETRIC PART.
  ...
  IF(ILS <= ITHRESHOLD .OR. .NOT.S%LUSEFLT) THEN
    ...
    IF (LLDOUBLE) THEN
       CALL GEMM('N','N',KIFC,ILS,KDGLU,1.0_JPRD,ZB,KIFC,S%FA(KMLOC)%RPNMS,KDGLU,&
            & 0._JPRD,ZCS,KIFC)
    ELSE
       IF(KM>=1)THEN ! DGEM for the mean to improve mass conservation
          CALL GEMM('N','N',KIFC,ILS,KDGLU,1.0_JPRM,ZB,KIFC,S%FA(KMLOC)%RPNMS,KDGLU,&
               & 0._JPRM,ZCS,KIFC)
       ELSE
          ...
          DO I3=1,I1
             ...
             END DO
          END DO
          CALL GEMM('N','N',KIFC,ILS,KDGLU,1.0_JPRD,ZB_D,KIFC,ZRPNMS,KDGLU,&
               & 0._JPRD,ZCS_D,KIFC)
          DO J=1,ILS
             IFLD=0
             DO JK=1,KFC,ISKIP
